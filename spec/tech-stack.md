...
- **Fetch is role-scoped, not a full dump.** A source adapter's fetch method takes a role/keyword query built from the user's target roles (`SearchPreference`) and asks the source itself to filter — it never pulls a source's entire board and filters client-side. Only postings a source reports as matching that query get normalized and written to `JobPosting`; nothing else touches the DB. A source whose API can't be queried by keyword/role isn't a fit for this project's source list.
- **Prefilter before LLM, always.** Scoring a posting with the LLM is the expensive step in the pipeline. Nothing reaches the scorer without passing the cheap rule-based prefilter first — this prefilter runs on the already role-scoped postings from the fetch step, checking the *other* preferences (work mode, experience, pay, location, sanity rules), not role relevance again.
- **User filters are data, not code.** Target roles/keywords, work mode (remote/on-site/hybrid), pay range, location(s), and similar search preferences are stored per-user in a `SearchPreference` table (`packages/db`) and read by both the source-fetch step and the prefilter at run time. They are never hardcoded constants in `packages/sources` or `packages/scoring` — a user changing a filter in the UI must not require a deploy.
