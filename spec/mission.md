...
2. Takes user-set search preferences — target roles/keywords, work mode (remote / on-site / hybrid), pay range, location(s), and similar filters — that scope what's worth fetching and scoring at all.
3. Polls job sources on a schedule, querying each source for the roles the user has described, and normalizes and saves only the matching postings into one schema.
4. Filters cheaply with rules (including the user's other search preferences), then scores survivors with an LLM against each resume variant to find the best match.
...
- **Match quality over volume.** A smaller number of well-matched suggestions beats a flooded queue. Source queries are scoped to the roles the user describes at fetch time — nothing unrelated gets ingested in the first place — and the prefilter, the user's own filters, and the scorer all exist on top of that to protect the user's review time, not to maximize postings ingested.
...
- The queue only shows jobs that fit the roles the user is searching for, the user's stated filters (work mode, pay, location), and the resume match.
